...
def cities_output():
  ...
  with db:
    ...
    #CHANGED!
    cur.execute("SELECT predict_protest, jpg, city, hourly FROM (%s) WHERE hourly IN ('%s');" %(city_t, hourly))
  ...
  return render_template("output.html", picture = picture, the_result = the_result, the_city = the_city, city_table = city_table, the_date = the_date)
